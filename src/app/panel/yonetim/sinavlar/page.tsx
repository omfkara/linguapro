...
export default async function AdminExamsPage() {
  const examList = await db
    ...
    .where(eq(exams.type, "COURSE_EXAM"))
  ...
  return (
    <div>
      <DashboardPageHeader
        title="Kurs Sınavları"
        description="Kurs sonu değerlendirme sınavlarını yönetin."
        action={
          <LinkButton href="/panel/yonetim/sinavlar/yeni" size="sm">
      ...
      {examList.length === 0 ? (
        <EmptyState
          ...
          description="Bir kurs seçip ilk sınavınızı oluşturun."
          action={
            <LinkButton href="/panel/yonetim/sinavlar/yeni" size="sm">
              Sınav Oluştur
            </LinkButton>
          ...
          {examList.map((e) => (
            <div key={e.id} className="rounded-2xl border border-ink-100 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 flex items-center gap-2 border-t border-ink-100 pt-4">
                <Link
                  href={`/panel/yonetim/sinavlar/${e.id}`}
